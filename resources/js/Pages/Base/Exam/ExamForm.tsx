...
import { BranchType, Classes, Subject } from '@/types';
...
const ExamForm = ({
    ...
    const [subjects, setSubjects] = useState<Subject[]>(
        exam && exam.class_id
            ? classes.filter((c) => c.class_id === exam.class_id)[0].subjects
            : [],
    );
    ...
    const filterSubjectByClass = (value: string) => {
        setSubjects(classes.filter((c) => c.class_id === value)[0].subjects);
    };

    const handleSubmit = (event: React.FormEvent) => {
        event.preventDefault();
        if (exam) {
            put(route('exam.update', exam.exam_id), {
                onSuccess: () => {
            ...
            <div className="mb-4">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-6">
                    <div>
                        ...
                        >
                            <PopoverTrigger className="w-full pt-1" asChild>
                                <Button
                                    type="button"
                                    variant="outline"
                                    data-empty={!data.start_date}
                    ...
                    <div>
                        ...
                        >
                            <PopoverTrigger className="w-full p-1" asChild>
                                <Button
                                    type="button"
                                    variant="outline"
                                    data-empty={!data.start_date}
            ...
            <div className="mb-4 grid grid-cols-1 md:grid-cols-3">
                <div className="col-span-1 md:col-span-2">
                    ...
                    <Textarea
                        ...
                        onChange={(e) =>
                            setData('exam_description', e.target.value)
                        }
                        className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-gray-200 ${
                            errors.exam_description ? 'border-red-500' : ''
                        }`}
                    ></Textarea>
